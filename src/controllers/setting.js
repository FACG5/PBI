...
exports.get = (req, res) => {
  res.render('setting', { cssFile: 'panelSetting', jsFile: 'setting' });
};
  ...
  admin.findAll().then((result) => {
    ...
    bcryptjs.compare(currentPassword, currentPasswordDatabase, (err, result) => {
      ...
      return bcryptjs.hash(password, 10, (err, password) => {
        ...
        return admin.update(newAdminData, { where: { id: 1 } }).then(() => res.send(JSON.stringify({ err: null, message: 'تم تحديث البيانات بنجاح' }))).catch(error => next(error));
      });
    });
  });
};
